...
import com.example.healthconnect.codelab.presentation.component.cadenceSeries
  ...
  if (uiState != ExerciseSessionDetailViewModel.UiState.Uninitialized) {
    LazyColumn(
      ...
      if (!permissionsGranted) {
        ...
        cadenceSeries(
          labelId = R.string.cadence,
          series = sessionMetrics.cadenceRecord
        )
        speedSeries(
          labelId = R.string.speed_stats,
...
fun ExerciseSessionScreenPreview() {
  HealthConnectTheme {
    ...
    val sessionMetrics = ExerciseSessionData(
      ...
      cadenceRecord = generateCadenceSeries(),
...
private fun generateCadenceSeries(): List<StepsCadenceRecord> {
  val data = mutableListOf<StepsCadenceRecord.Sample>()
  val end = ZonedDateTime.now()
  var time = ZonedDateTime.now()
  for (index in 1..10) {
    time = end.minusMinutes(index.toLong())
    data.add(
      StepsCadenceRecord.Sample(
        time = time.toInstant(),
        rate = Random.nextDouble(80.0, 180.0) // Cadence rate in steps per minute
      )
    )
  }
  return listOf(
