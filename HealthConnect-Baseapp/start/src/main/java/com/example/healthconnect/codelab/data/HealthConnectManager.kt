...
//  suspend fun estimateCadence(start: Instant, end: Instant): Double {
//    val stepsData = readData<StepsRecord>(TimeRangeFilter.between(start, end))
//    val totalSteps = stepsData.sumOf { it.count }
//    val totalTimeInMinutes = Duration.between(start, end).toMinutes().toDouble() // Convert Long to Double
//    return if (totalTimeInMinutes > 0) (totalSteps / totalTimeInMinutes) * 2 else 0.0
//  }
...
//  }
  ...
  suspend fun readAssociatedSessionData(
    ...
    val cadenceData = readData<StepsCadenceRecord>(timeRangeFilter, dataOriginFilter)
